...
interface BackendDevelopersClientProps {
  ...
  backendDevelopersData: BasePageData
}

export default function BackendDevelopersClient({ faqs, backendDevelopersData }: BackendDevelopersClientProps) {
  const hero = backendDevelopersData?.hero
  const badge = hero?.badge
  const BadgeIcon = getIconComponent(badge?.iconName) || Server
  const badgeIcon = <BadgeIcon className="w-4 h-4" strokeWidth={2} />

  const services = useMemo(() => 
    (backendDevelopersData?.services ?? []).map(service => {
      const IconComponent = getIconComponent(service.iconName) || Code
      return {
        ...service,
        icon: <IconComponent className="w-8 h-8" strokeWidth={2} />
      }
    }), [backendDevelopersData?.services]
  )

  const whyChooseUs = useMemo(() => 
    (backendDevelopersData?.whyChooseUs ?? []).map(item => {
      const IconComponent = getIconComponent(item.iconName) || Code
      return {
        ...item,
        icon: <IconComponent className="w-7 h-7" strokeWidth={2} />
      }
    }), [backendDevelopersData?.whyChooseUs]
  )

  const industries = useMemo(() => 
    (backendDevelopersData?.industries ?? []).map(industry => {
      const IconComponent = getIconComponent(industry.iconName) || Building2
      return {
        ...industry,
        icon: <IconComponent className="w-7 h-7" strokeWidth={2} />
      }
    }), [backendDevelopersData?.industries]
  )

  const processSteps = useMemo(() => 
    (backendDevelopersData?.processSteps ?? []).map(step => {
      const IconComponent = getIconComponent(step.iconName) || Target
      return {
        ...step,
        icon: <IconComponent className="w-6 h-6" strokeWidth={2} />
      }
    }), [backendDevelopersData?.processSteps]
  )

  const technologiesList = Array.isArray(backendDevelopersData?.technologies) ? [...backendDevelopersData.technologies] : []
