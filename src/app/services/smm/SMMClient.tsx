...
interface SMMClientProps {
  faqs: Array<{ question: string; answer: string }>
  smmData: BasePageData
}

export default function SMMClient({ faqs, smmData }: SMMClientProps) {
  ...
  const previewServices = useMemo(() => 
    smmData.services.slice(0, 4).map(service => {
      const IconComponent = getIcon(service.iconName)
      return {
        ...service,
        icon: IconComponent ? <IconComponent className="w-8 h-8" strokeWidth={2} /> : null
  ...
  const BadgeIcon = getIcon(smmData.hero.badge.iconName)
  ...
  return (
    <>
      ...
      <section className="py-20 bg-white section-reveal">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 scroll-animate">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto section-desc-reveal">
              Serving diverse industries with SMM solutions
            </p>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 card-grid-direction">
            {smmData.useCases.map((useCase, index) => (
              <div
                key={index}
      ...
      />
      <GetQuoteSection
        title={smmData.getQuote.title}
        hollowText={smmData.getQuote.hollowText}
        description={smmData.getQuote.description}
        primaryCTA={{
          text: 'Call Us',
          type: 'tel',
          href: '+916354326412'
      ...
      <RelatedLinks 
        ...
        links={getServiceRelatedLinks('smm')} 
        columns={3}
      />
      <ContactFormModal 
