...
interface PPCClientProps {
  faqs: Array<{ question: string; answer: string }>
}

export default function PPCClient({ faqs }: PPCClientProps) {
  ...
  const services = ppcData.services.map(service => {
  ...
  const whyChooseUs = ppcData.whyChooseUs.map(item => {
  ...
  const processSteps = ppcData.processSteps.map(step => {
  ...
  const industries = ppcData.industries.map(industry => {
  ...
  const benefits = ppcData.benefits.map(benefit => {
  ...
  const previewServices = ppcData.services.slice(0, 4).map(service => {
    const IconComponent = getIcon(service.iconName)
    return {
      ...service,
      icon: IconComponent ? <IconComponent className="w-8 h-8" strokeWidth={2} /> : null
  ...
  const BadgeIcon = getIcon(ppcData.hero.badge.iconName)
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="scroll-animate">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#5e2cb6]/10 rounded-full mb-6">
                ...
                <span className="text-sm font-semibold text-[#5e2cb6]">{ppcData.hero.badge.text}</span>
              </div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-gray-900 mb-6 leading-tight">
                {ppcData.hero.title}{' '}
                <span className="hollow-text-brand block mt-2">
                  {ppcData.hero.hollowText}
                </span>
              </h1>
              <p className="text-base md:text-lg text-gray-600 mb-8 leading-relaxed">
                {ppcData.hero.description}
              </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              We deliver data-driven pay-per-click campaigns that drive immediate, targeted traffic and measurable results.
            </p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 scroll-animate">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Serving diverse industries with PPC advertising solutions
            </p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Discover how pay-per-click advertising can drive immediate traffic and results
            </p>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Explore how businesses use pay-per-click advertising to drive traffic and conversions
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {ppcData.useCases.map((useCase, index) => (
              <div
                key={index}
      ...
      />
      <GetQuoteSection
        title={ppcData.getQuote.title}
        hollowText={ppcData.getQuote.hollowText}
        description={ppcData.getQuote.description}
        primaryCTA={{
          text: 'Call Us',
          onClick: () => setIsContactModalOpen(true)
        }}
