...
interface EmailMarketingClientProps {
  ...
  emailMarketingData: BasePageData
}

export default function EmailMarketingClient({ faqs, emailMarketingData }: EmailMarketingClientProps) {
  ...
  const services = useMemo(() => 
    emailMarketingData.services.map(service => {
      const IconComponent = getIcon(service.iconName)
      return {
        ...service,
        icon: IconComponent ? <IconComponent className="w-8 h-8" strokeWidth={2} /> : null
      }
    }), [emailMarketingData.services]
  )

  const whyChooseUs = useMemo(() => 
    emailMarketingData.whyChooseUs.map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-7 h-7" strokeWidth={2} /> : null
      }
    }), [emailMarketingData.whyChooseUs]
  )

  const processSteps = useMemo(() => 
    emailMarketingData.processSteps.map(step => {
      const IconComponent = getIcon(step.iconName)
      return {
        ...step,
        icon: IconComponent ? <IconComponent className="w-6 h-6" strokeWidth={2} /> : null
      }
    }), [emailMarketingData.processSteps]
  )

  const industries = useMemo(() => 
    emailMarketingData.industries.map(industry => {
      const IconComponent = getIcon(industry.iconName)
      return {
        ...industry,
        icon: IconComponent ? <IconComponent className="w-7 h-7" strokeWidth={2} /> : null
      }
    }), [emailMarketingData.industries]
  )

  const benefits = useMemo(() => 
    emailMarketingData.benefits.map(benefit => {
      const IconComponent = getIcon(benefit.iconName)
      return {
        ...benefit,
        icon: IconComponent ? <IconComponent className="w-6 h-6" strokeWidth={2} /> : null
      }
    }), [emailMarketingData.benefits]
  )

  const previewServices = useMemo(() => 
    emailMarketingData.services.slice(0, 4).map(service => {
      const IconComponent = getIcon(service.iconName)
      return {
        ...service,
        icon: IconComponent ? <IconComponent className="w-8 h-8" strokeWidth={2} /> : null
  ...
  return (
    <>
      ...
      />
      <RelatedLinks title="Related Services" links={getServiceRelatedLinks('email-marketing')} columns={3} />
      <ContactFormModal 
