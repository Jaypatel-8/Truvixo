...
interface SaaSClientProps {
  ...
  saasData: BasePageData
}

export default function SaaSClient({ faqs, saasData }: SaaSClientProps) {
  ...
  const previewServices = useMemo(() => 
    saasData.services.slice(0, 4).map(service => {
      const IconComponent = getIcon(service.iconName)
      return {
        ...service,
        icon: IconComponent ? <IconComponent className="w-8 h-8" strokeWidth={2} /> : null
  ...
  const BadgeIcon = getIcon(saasData.hero.badge.iconName)
    ...
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="scroll-animate">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#5e2cb6]/10 rounded-full mb-6">
                ...
                <span className="text-sm font-semibold text-[#5e2cb6]">{saasData.hero.badge.text}</span>
              </div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-gray-900 mb-6 leading-tight">
                {saasData.hero.title}{' '}
                <span className="hollow-text-brand block mt-2">
                  {saasData.hero.hollowText}
                </span>
              </h1>
              <p className="text-lg md:text-xl text-gray-600 mb-8 leading-relaxed">
                {saasData.hero.description}
              </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-4">
              {saasData.servicesTitle}{' '}
              <span className="hollow-text-brand">
                {saasData.servicesHollowText}
              </span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 card-grid-direction">
            {services.map((service, index) => (
              <div
                ...
                style={{ ['--card-accent' as string]: service.color, borderColor: service.color + '40' }}
              >
      ...
      <section className="py-20 bg-gray-50 section-reveal">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-4">
              {saasData.whyChooseTitle}{' '}
              <span className="hollow-text-brand">
                {saasData.whyChooseHollowText}
              </span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 card-grid-direction">
            {whyChooseUs.map((item, index) => (
              <div
                key={index}
                className="scroll-animate-scale card-hover card-hover-dark group bg-white rounded-2xl p-8 border-2 border-gray-100 relative overflow-hidden"
                style={{ ['--card-accent' as string]: item.color, borderColor: item.color + '40' }}
              >
                <div className="relative z-10 card-inner-reveal">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 card-grid-direction">
            {industries.map((industry, index) => (
              <div
                key={index}
                className="scroll-animate-scale card-hover card-hover-dark bg-white rounded-xl p-6 text-center border-2 border-gray-100 relative overflow-hidden"
                style={{ ['--card-accent' as string]: industry.color, borderColor: industry.color + '40' }}
              >
      ...
      <section className="py-20 bg-white section-reveal">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-4">
              {saasData.benefitsTitle}{' '}
              <span className="hollow-text-brand">
                {saasData.benefitsHollowText}
              </span>
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto section-desc-reveal">
      ...
      <section className="py-20 bg-gray-50 section-reveal">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 scroll-animate">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-4">
              {saasData.useCasesTitle}{' '}
              <span className="hollow-text-brand">
                {saasData.useCasesHollowText}
              </span>
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto section-desc-reveal">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 card-grid-direction">
            {saasData.useCases.map((useCase, index) => (
              <div
                key={index}
      ...
      <Technologies technologies={[...saasData.technologies]} />
      <ProcessDiagram 
        title={saasData.processTitle}
        subtitle={saasData.processSubtitle}
        steps={processSteps}
      />
      <FAQDropdown faqs={faqs} />
      <ContactSection 
        title={saasData.contact.title}
        description={saasData.contact.description}
      />
      <GetQuoteSection
        title={saasData.getQuote.title}
        hollowText={saasData.getQuote.hollowText}
        description={saasData.getQuote.description}
        primaryCTA={{
          text: 'Call Us',
          type: 'tel',
          href: '+916354326412'
