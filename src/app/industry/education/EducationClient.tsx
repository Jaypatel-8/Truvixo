...
interface EducationClientProps {
  ...
  educationData: BasePageData
}

export default function EducationClient({ faqs, educationData }: EducationClientProps) {
  ...
  const services = useMemo(() => 
    (educationData?.services ?? []).map(service => {
  ...
  const whyChooseUs = useMemo(() => 
    (educationData?.whyChooseUs ?? []).map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-7 h-7" strokeWidth={2} /> : null
      }
    }), [educationData?.whyChooseUs]
  )

  const processSteps = useMemo(() => 
    (educationData?.processSteps ?? []).map(step => {
  ...
  return (
    <>
      ...
      <Technologies technologies={Array.isArray(educationData?.technologies) ? [...educationData.technologies] : []} />
      <ProcessDiagram 
      ...
      />
      <RelatedLinks title="Related Industries & Services" links={getIndustryRelatedLinks('education')} columns={3} />
      <ContactFormModal 
