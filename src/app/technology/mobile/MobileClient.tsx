...
interface MobileClientProps {
  ...
  mobileTechnologyData: BasePageData
}

export default function MobileClient({ faqs, mobileTechnologyData }: MobileClientProps) {
  ...
  const technologies = useMemo(() => 
    mobileTechnologyData.technologies.map(tech => {
      const IconComponent = getIcon(tech.iconName)
      return {
        ...tech,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [mobileTechnologyData.technologies]
  )

  const whatWeBuild = useMemo(() => 
    mobileTechnologyData.whatWeBuild.map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [mobileTechnologyData.whatWeBuild]
  )

  const benefits = useMemo(() => 
    mobileTechnologyData.benefits.map(benefit => {
      const IconComponent = getIcon(benefit.iconName)
      return {
        ...benefit,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [mobileTechnologyData.benefits]
  )

  const whyChooseUs = useMemo(() => 
    mobileTechnologyData.whyChooseUs.map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [mobileTechnologyData.whyChooseUs]
  )
  ...
  return (
    <main className="min-h-screen bg-white overflow-hidden pt-20">
      ...
      <GetQuoteSection
        ...
        hollowText="with Us"
        description="Get in touch and let's discuss how we can help you build native and cross-platform mobile applications."
        primaryCTA={{
          text: 'Call Us',
