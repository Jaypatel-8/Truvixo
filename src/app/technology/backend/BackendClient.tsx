...
import { backendTechnologyData } from '@/lib/staticData/technology/backend'
...
export default function FrontendClient({ faqs }: FrontendClientProps) {
  ...
  const technologies = backendTechnologyData.technologies.map(tech => {
  ...
  const whatWeBuild = backendTechnologyData.whatWeBuild.map(item => {
  ...
  const benefits = backendTechnologyData.benefits.map(benefit => {
  ...
  const whyChooseUs = backendTechnologyData.whyChooseUs.map(item => {
    const IconComponent = getIcon(item.iconName)
    return {
      ...item,
      icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="scroll-animate">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
              {backendTechnologyData.cta.title}
            </h2>
            <button
            ...
            >
              {backendTechnologyData.cta.buttonText}
