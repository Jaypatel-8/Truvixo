...
interface CloudDevOpsClientProps {
  faqs: Array<{ question: string; answer: string }>
  cloudDevOpsTechnologyData: BasePageData
}

export default function CloudDevOpsClient({ faqs, cloudDevOpsTechnologyData }: CloudDevOpsClientProps) {
  ...
  const technologies = useMemo(() => 
    cloudDevOpsTechnologyData.technologies.map(tech => {
      const IconComponent = getIcon(tech.iconName)
      return {
        ...tech,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [cloudDevOpsTechnologyData.technologies]
  )

  const whatWeBuild = useMemo(() => 
    cloudDevOpsTechnologyData.whatWeBuild.map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [cloudDevOpsTechnologyData.whatWeBuild]
  )

  const benefits = useMemo(() => 
    cloudDevOpsTechnologyData.benefits.map(benefit => {
      const IconComponent = getIcon(benefit.iconName)
      return {
        ...benefit,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [cloudDevOpsTechnologyData.benefits]
  )

  const whyChooseUs = useMemo(() => 
    cloudDevOpsTechnologyData.whyChooseUs.map(item => {
      const IconComponent = getIcon(item.iconName)
      return {
        ...item,
        icon: IconComponent ? <IconComponent className="w-6 h-6" /> : null
      }
    }), [cloudDevOpsTechnologyData.whyChooseUs]
  )
